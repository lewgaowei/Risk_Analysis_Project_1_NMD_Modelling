...
# Note: Dataset ends 30-Dec-2023 (last available observation); used as effective
...
# 2. **Portfolio KM (Aggregate Proxy)** - Non-parametric, data-driven
...
# ── Survival proxy construction ────────────────────────────────────────────────
# For parametric models (Weibull, Log-Normal, Log-Logistic) we need a target
# series that behaves like a proper survival function: starts at 1, is
# non-increasing, and ends at some value >= 0.
#
# PROBLEM with naive approach (Balance / max_balance):
#   - Balance is GROWING over the period, so this series rises toward 1 then
#     potentially falls — it is NOT monotonically decreasing and does not
#     represent survival probability. Fitting survival functions to this gives
#     meaningless parameter estimates.
#
# SOLUTION — Net-decay cumulative survival proxy:
#   Step 1: Compute daily net attrition = max(0, Outflow - Inflow)
#   Step 2: Daily net survival = 1 - net_attrition / Balance
#   Step 3: Cumulative product gives S(t) = Π_{i=0}^{t} (1 - λ_i)
#   This is exactly how a product-limit (KM-style) estimator works and gives
#   a properly monotone decreasing survival curve starting at 1.
# ──────────────────────────────────────────────────────────────────────────────

nmd_data['net_attrition'] = np.maximum(
    0, nmd_data['Outflow'] - nmd_data['Inflow']
)
nmd_data['daily_net_survival'] = 1 - nmd_data['net_attrition'] / nmd_data['Balance']
# Clip to [0,1] to guard against edge cases
nmd_data['daily_net_survival'] = nmd_data['daily_net_survival'].clip(0, 1)

# Cumulative product = empirical survival function S(t)
nmd_data['survival_proxy'] = nmd_data['daily_net_survival'].cumprod()

# Create event data (for reference)
nmd_data['balance_change'] = nmd_data['Balance'].diff()
nmd_data['withdrawal_event'] = (nmd_data['balance_change'] < 0).astype(int)

print("\n" + "="*80)
print("SURVIVAL DATA PREPARATION — NET ATTRITION CUMULATIVE PRODUCT")
print("="*80)
print(f"Initial Balance:              {initial_balance:,.2f}")
print(f"Final Balance:                {nmd_data.iloc[-1]['Balance']:,.2f}")
print(f"Growth over period:           {(nmd_data.iloc[-1]['Balance']/initial_balance - 1)*100:.2f}%")
print(f"Minimum Balance:              {nmd_data['Balance'].min():,.2f}")
print(f"Days with net withdrawal:     {nmd_data['withdrawal_event'].sum()}")
print(f"")
print(f"Survival proxy S(0):          {nmd_data['survival_proxy'].iloc[0]:.4f}  (should be ~1.0)")
print(f"Survival proxy S(1Y):         {nmd_data['survival_proxy'].iloc[min(365, len(nmd_data)-1)]:.4f}")
print(f"Survival proxy S(5Y/end):     {nmd_data['survival_proxy'].iloc[-1]:.4f}")
print(f"Method: Cumulative product of daily (1 - net_attrition/balance)")
...
# survival_proxy is the net-attrition cumulative product survival function:
# - Starts at 1, monotonically decreasing, properly behaves like S(t)
days = nmd_data['days_since_start'].values
balance_normalized = nmd_data['survival_proxy'].values  # Corrected survival target
...
# %% [markdown]
# ## 4. MODEL 2: Portfolio KM (Aggregate Proxy)

# %%
print("\n" + "="*80)
print("MODEL 2: PORTFOLIO KM (AGGREGATE PROXY)")
print("="*80)
print("\nPortfolio-level adaptation of the Kaplan-Meier product-limit concept.")
print("Not a formal KM estimator — true KM requires individual-level censored")
print("event-time data (per-account open/close records). Here we adapt the KM idea")
print("to aggregate portfolio balances using cumulative minimum retention:")
print("  Retention(t) = cumulative_min(Balance(t)) / Balance(t)")
print("This gives a conservative lower bound on deposit survival, forced monotone-")
print("decreasing to satisfy survival function requirements.")
...
print(f"\nPortfolio KM (Aggregate Proxy):")
print(f"  Method:              Cumulative minimum / current balance, forced monotone")
print(f"  Interpretation:      CONSERVATIVE — lower bound on empirical deposit retention")
...
print(f"  Basis:               Portfolio-level KM adaptation (not classical KM)")
...
# Calculate mean daily decay rate — net CDR, consistent with survival_proxy
# Net CDR = max(0, Outflow - Inflow) / Balance (already in net_attrition column)
# Using gross daily_decay_rate (Outflow/Balance from Phase 1a) would overstate decay
# and make this baseline incomparable to the parametric models above.
mean_daily_decay = (nmd_data['net_attrition'] / nmd_data['Balance']).mean()
...
print(f"  CDR basis:           Net (max(0, Outflow-Inflow) / Balance)")
...
# %%
print("\n" + "="*80)
print("MODEL 6: CONSTANT/FLAT SURVIVAL (FOR COMPARISON ONLY — NOT FOR SLOTTING)")
print("="*80)
print("\nAssumption: Stable core deposits have FLAT survival (zero decay).")
print("  S(t) = constant (no decay at all)")
print("  Based on core ratio from Phase 1c")
print("")
print("⚠️  WARNING: This model is shown for comparison purposes ONLY.")
print("   It CANNOT be used for cash flow slotting because:")
print("   - Marginal decay [S(t_start) - S(t_end)] = 0 for all intermediate buckets")
print("   - This means zero cash flow in every bucket between O/N and 5Y")
print("   - All core deposits would pile into the residual 5Y bucket")
print("   - This violates the Basel requirement for a meaningful maturity distribution")
print("   It is included here to show the upper bound of stability assumptions.")
...
if km_converged:
    ...
    comparison_data.append({
        'Model': '3. Portfolio KM (Aggregate Proxy)',
        'S(1Y)': s1y_km,
        'S(5Y)': s5y_km,
        'R²': np.nan,
        'RMSE': np.nan,
...
if flat_converged:
    comparison_data.append({
        ...
        'R²': np.nan,
        'RMSE': np.nan,
        'Interpretation': f'COMPARISON ONLY — zero intermediate CFs, not valid for slotting. Stable core ({core_ratio*100:.1f}%)'
    })
...
# Change SELECTED_MODEL here to switch which curve feeds Phase 2 cashflow slotting.
# Valid options (must match a converged model):
#   'portfolio_km'  — Portfolio KM (Aggregate Proxy): cumulative-min retention (conservative)
#   'weibull'       — Parametric, flexible hazard shape
#   'lognormal'     — Parametric, fat-tailed
#   'loglogistic'   — Parametric, non-monotonic hazard
#   'exponential'   — Baseline constant-hazard model
#
# CURRENT SELECTION: 'portfolio_km'
# RATIONALE:
#   1. Purely data-driven — no distributional assumptions that can be challenged
#   2. Conservative (lower S(t)) → more cash flow in short buckets → higher
#      measured IRRBB exposure — prudent from a regulatory standpoint
#   3. Consistent with BCBS 2016 principle of conservative NMD modelling
#   4. Produces a monotone-decreasing S(t) by construction
#   5. Limitations acknowledged: portfolio-level KM adaptation, not a formal KM
#      estimator. A parametric model (Weibull) would be preferred if fitting
#      individual deposit cohort data were available.
# ──────────────────────────────────────────────────────────────────────────────
SELECTED_MODEL = 'portfolio_km'  # ← change here to switch model

# Map selection to converged model objects
_model_map = {
    'portfolio_km': ('3. Portfolio KM (Aggregate Proxy)', km_converged),
    'weibull':         ('2. Weibull',            weibull_converged),
    'lognormal':       ('4. Log-Normal',         lognorm_converged),
    'loglogistic':     ('5. Log-Logistic',       loglogistic_converged),
    'exponential':     ('1. Exponential (Baseline)', exponential_converged),
}

_label, _converged = _model_map.get(SELECTED_MODEL, ('1. Exponential (Baseline)', True))

if not _converged:
    print(f"\n⚠️  Selected model '{SELECTED_MODEL}' did not converge.")
    print(f"   Falling back to Exponential baseline.")
    SELECTED_MODEL = 'exponential'
    _label = '1. Exponential (Baseline)'

# Resolve best_model and best_idx
best_model = _label
if best_model in comparison_df['Model'].values:
...
print(f"\nSELECTED MODEL: {SELECTED_MODEL.upper()}  →  '{best_model}'")
print(f"  S(1Y):  {comparison_df.loc[best_idx, 'S(1Y)_%']:.2f}%")
print(f"  S(5Y):  {best_s5y:.2f}%")
print(f"\nRationale: Conservative non-parametric floor. No distributional")
print(f"assumptions. Prudent for IRRBB regulatory reporting (BCBS 2016 para 113).")
print(f"\nTo switch models: set SELECTED_MODEL variable above to one of:")
for k, (lbl, conv) in _model_map.items():
    conv_str = "converged" if conv else "DID NOT CONVERGE"
    print(f"  '{k}' → {lbl} ({conv_str})")

# Use Portfolio KM as primary recommendation (most conservative/worst case)
if km_converged:
    print(f"\nRecommended Model: PORTFOLIO KM AGGREGATE PROXY (CONSERVATIVE)")
    print(f"  S(1Y):               {survival_km_full[365]*100 if len(survival_km_full) > 365 else 0:.2f}%")
    print(f"  S(5Y):               {best_s5y:.2f}%")
    print(f"\nWhy Portfolio KM (Aggregate Proxy)?")
    print(f"  ✓ Non-parametric, PURELY DATA-DRIVEN")
    print(f"  ✓ No distributional assumptions")
    print(f"  ✓ CONSERVATIVE approach (worst-case retention scenario)")
    print(f"  ✓ Based on cumulative minimum balance retention (lower bound)")
    print(f"  ✓ S(5Y)={best_s5y:.2f}% is among the most conservative estimates (prudent)")
    print(f"  ✓ Portfolio-level KM adaptation — not classical KM (which requires")
    print(f"    individual-level censored event data per account)")
else:
    if weibull_converged:
        best_model = '2. Weibull'
        best_idx = comparison_df[comparison_df['Model'] == best_model].index[0]
        best_s5y = comparison_df.loc[best_idx, 'S(5Y)_%']
        print(f"\nPortfolio KM not converged. Using Weibull:")
        print(f"  S(5Y):               {best_s5y:.2f}%")
    else:
...
if km_converged:
    ax.plot(days_km_full[:len(days_pred)] / 365, survival_km_full[:len(days_pred)] * 100,
            linewidth=2.5, label='Portfolio KM (Aggregate Proxy)', alpha=0.8)
...
# %%
# Select recommended model for Phase 2 - USE PORTFOLIO KM (CONSERVATIVE)
if km_converged:
    # Pad Portfolio KM to full length
    ...
    model_name = 'Portfolio KM'
    model_params = {
        'method': 'Portfolio KM (Aggregate Proxy) — cumulative-min retention',
        'description': 'Cumulative minimum / current balance, portfolio-level KM adaptation',
        'conservative': True
    }
...
if km_converged:
    ...
    all_models_df['S(t)_Portfolio_KM'] = km_padded
...
for days, tenor_name in key_tenors:
    if days < len(survival_recommended):
        ...
        if km_converged and days < len(survival_km_full):
            row['S(t)_Portfolio_KM'] = survival_km_full[days]
        if lognorm_converged:
...
config_data['phase_1b_survival_model'] = {
    ...
    'interpretation': 'Portfolio KM (Aggregate Proxy) — conservative cumulative-min retention floor' if model_name == 'Portfolio KM' else ('Decreasing hazard - deposits more stable over time' if model_name == 'Weibull' else 'Standard survival model')
}
...
print("\n7. stress_periods_identified.csv  ← NEW")
print("   → Daily stress flags (net CDR P85/P90/P95 thresholds)")
print("   → Input for Phase 1c stressed core estimate under rate-shock scenarios")
...
if model_name == 'Portfolio KM':
    print(f"\nPortfolio KM (Aggregate Proxy) Characteristics:")
    print(f"  Method:            Cumulative minimum retention, forced monotone-decreasing")
    print(f"  Interpretation:    CONSERVATIVE — lower bound on empirical deposit retention")
    print(f"  Advantage:         No distributional assumptions, purely data-driven")
    print(f"  Note:              Portfolio-level KM adaptation — not classical KM")
    print(f"                     (classical KM requires individual-level censored account data)")
...
# %% [markdown]
# ## 13. Stress Period Export — Input for Phase 1c Stressed Core Estimate
#
# Phase 1c v2 requires stress period flags to compute a stressed core ratio
# under rate-shock scenarios (Basel IRRBB requirement). We identify days where
# net CDR exceeded the P90 threshold — these high-decay episodes serve as a
# proxy for a rate-stress environment.
#
# Uses net CDR (max(0, Outflow-Inflow)/Balance), consistent with the survival
# proxy above. The archived decay model used OLS-predicted gross CDR with
# rolling_30d_decay as a feature (inflated R² artificially) — not used here.

# %%
print("\n" + "="*80)
print("STRESS PERIOD EXPORT — INPUT FOR PHASE 1C STRESSED CORE ESTIMATE")
print("="*80)
print("\nPurpose: Identify high-decay days as proxy for rate-stress episodes")
print("Method:  Flag days where net CDR > P90 threshold (net attrition / Balance)")

# Net CDR — uses net_attrition already computed earlier in this script
net_cdr_series = (nmd_data['net_attrition'] / nmd_data['Balance']).clip(0, 1)

p85_threshold = net_cdr_series.quantile(0.85)
p90_threshold = net_cdr_series.quantile(0.90)
p95_threshold = net_cdr_series.quantile(0.95)

n_stress_p90 = int((net_cdr_series > p90_threshold).sum())
n_total       = len(net_cdr_series)

print(f"\n  Net CDR P85 threshold:   {p85_threshold:.6f}")
print(f"  Net CDR P90 threshold:   {p90_threshold:.6f}  "
      f"({n_stress_p90} / {n_total} days = {n_stress_p90/n_total*100:.1f}% of history flagged)")
print(f"  Net CDR P95 threshold:   {p95_threshold:.6f}")

stress_export = pd.DataFrame({
    'Date':       nmd_data['Date'].values,
    'Balance':    nmd_data['Balance'].values,
    'NetCDR':     net_cdr_series.values,
    'Stress_P85': net_cdr_series.values > p85_threshold,
    'Stress_P90': net_cdr_series.values > p90_threshold,
    'Stress_P95': net_cdr_series.values > p95_threshold,
})

stress_export.to_csv('stress_periods_identified.csv', index=False)

print(f"\n  Exported: stress_periods_identified.csv")
print(f"  Columns:  Date, Balance, NetCDR, Stress_P85, Stress_P90, Stress_P95")
print(f"  Phase 1c reads this file → minimum balance on Stress_P90 days")
print(f"  → becomes the stressed core floor for IRRBB scenario analysis")
