...
def get_shock_s2(t_years):
    """S2: -200bps parallel (zero floor enforced post-hoc on shocked rate)"""
    return -0.02

def get_shock_s3(t_years):
    """S3: Steepener - +200bps at shortest tenor, tapering to 0 at 5Y (Basel cap)"""
    return 0.02 * max(1 - t_years / 5, 0)

def get_shock_s4(t_years):
    """S4: Flattener - +200bps at t=0, tapering to 0 at 2Y pivot, then -100bps at 5Y"""
    if t_years <= 2:
        return 0.02 * (1 - t_years / 2)
    else:
        return -0.01 * (t_years - 2) / (5 - 2)
...
# Match Phase 3 convention: shocked rate ≥ 0  →  effective shock ≥ −base_rate
# Recover base zero rate at each midpoint from DF_Base (annual compounding)
nii_buckets['Base_Rate_bps'] = (
    (1 / nii_buckets['DF_Base']) ** (1 / nii_buckets['Midpoint_Years']) - 1
) * 10_000
# Cap downward shocks so the resulting rate stays non-negative
for col in ['Shock_S2_bps', 'Shock_S4_bps']:
    nii_buckets[col] = np.maximum(nii_buckets[col], -nii_buckets['Base_Rate_bps'])
...
# Create NII summary table — use Phase 3's exact scenario names for consistency
_eve_scen_names = eve_summary[eve_summary['Scenario'] != 'Base Case']['Scenario'].values
nii_summary = pd.DataFrame({
    'Scenario': _eve_scen_names,
...
# Use direct array access — avoids fragile string-matching that broke when
# Phase 3 scenario names differ from Phase 4 labels.
eve_data = eve_summary[eve_summary['Scenario'] != 'Base Case'].copy().reset_index(drop=True)

combined_summary = pd.DataFrame({
    'Scenario': eve_data['Scenario'].values,
    'ΔEVE': eve_data['ΔEVE'].values,
    'ΔEVE_%': eve_data['ΔEVE_%'].values,
    'ΔNII': [delta_nii_s1, delta_nii_s2, delta_nii_s3, delta_nii_s4]
})
